...
window.clickROI = function () {
  return {
    ...
    parallelTimer: null,
    // init
    setROIs: function setROIs(rois) {
      var _this = this;

      this.rois = rois;
      this.$watch("playbackSpeed", function (value) {
        var video = _this.$refs.video;
        video.playbackRate = value;
      });
    },
    moveScrub: function moveScrub(e) {
      if (this.mousedown) {// this.scrub(e);
      }
    },
    scrub: function scrub(e) {
      var video = this.$refs.video;
      var progress = this.$refs.progress;
      var scrubTime = e.offsetX / progress.offsetWidth * video.duration; // console.log(scrubTime);
    ...
    rewind: function rewind(seconds) {
      var video = this.$refs.video;
      var newTime = video.currentTime - seconds > 0 ? video.currentTime - seconds : 0;
      this.prevClicked = {
        x: 0,
        y: 0
      };
    ...
    initTimes: function initTimes() {
      var video = this.$refs.video;
      this.labelDuration = new Date(video.duration * 1000).toISOString().substr(11, 8);
    },
    startParallelTimer: function startParallelTimer() {
      var _this2 = this;

      this.parallelTimer = setInterval(function () {
        _this2.replayClickEffect();
      }, 20);
    },
    handleProgress: function handleProgress() {
      ...
      this.labelCurrentTime = new Date(video.currentTime * 1000).toISOString().substr(11, 8);
      this.initTimes();
    },
    play: function play() {
    ...
    clickedVideo: function clickedVideo(e, args) {
      var vid = this.$refs.video;
      var viewportOffset = vid.getBoundingClientRect();
      var time = Math.round(vid.currentTime * 100) / 100;
      var x = (e.clientX - viewportOffset.left) * this.originalSize.width / this.displaySize.width;
      var y = (e.clientY - viewportOffset.top) * this.originalSize.height / this.displaySize.height;
      this.prevClicked = {
        x: x,
        y: y
      };
      var type = "may_be_seen";
    ...
    removeRoi: function removeRoi(index, args) {
      if (args.isReplay) {
        alert("Niet mogelijk in replay modus");
        return;
      }

      var roi = this.rois[index];
      this.rois.splice(index, 1);

      var data = _objectSpread(_objectSpread({}, roi), {}, {
    ...
    clickEffect: function clickEffect(e) {
    ...
    hideAllAnimations: function hideAllAnimations() {
      document.querySelectorAll(".clickEffect").forEach(function (el) {
        el.parentElement.removeChild(el);
      });
    },
    replayClickEffect: function replayClickEffect() {
      var _this3 = this;

      var video = this.$refs.video;
      var rois = JSON.parse(JSON.stringify(this.rois)); // console.log(
      ...
      var toShow = rois.filter(function (point) {
        return point.time > _this3.prevTime && point.time < video.currentTime && point.x != _this3.prevClicked.x && point.y != _this3.prevClicked.y;
      });
      var viewportOffset = video.getBoundingClientRect();
      toShow.forEach(function (p) {
        var clientX = viewportOffset.left + p.x * _this3.displaySize.width / _this3.originalSize.width;
        var clientY = viewportOffset.top + p.y * _this3.displaySize.height / _this3.originalSize.height;
...
__webpack_require__(/*! /Users/joris/Development/HPT/poc_003/1_roi_hinting/resources/js/app.js */"./resources/js/app.js");
module.exports = __webpack_require__(/*! /Users/joris/Development/HPT/poc_003/1_roi_hinting/resources/css/app.css */"./resources/css/app.css");
